...
% solving
% 15 equations against the 14 joint force components plus Tin
...
% v = omega x r, r from a point of known velocity on the same link.
% A, D and G are pinned to ground.
velocityA = [0 0 0];
velocityD = [0 0 0];
velocityG = [0 0 0];
...
% C and F can each be reached along a second branch. Both residuals
% should be zero if the loop equations were assembled correctly.
...
% Same five free bodies as the statics block with sum(F) = m*a_Si and
% sum(M) = J*alpha. The moment form only holds about the COM.
...
% Mass moment of inertia about each link's own COM
J_AB = 1;
J_BEC = 1;
J_CD = 1;
J_EF = 1;
...
% With every mass equal to 1 the weight vectors above imply g = 1. Real
% masses need the weights rewritten as -m*g.
...
eqn15 = NForceA + NForceB + WAB == MassAB * accelerationS1;
eqn16 = cross(A-S1,NForceA) + cross(B-S1,NForceB) + NInputTorque == J_AB * alphaAB_vector;

% Equations for Link BEC
eqn17 = -NForceB + NForceC + NForceE + WBEC == MassBEC * accelerationS2;
eqn18 = cross(B-S2,-NForceB) + cross(C-S2,NForceC) + cross(E-S2,NForceE) == J_BEC * alphaBEC_vector;

% Equations for Link CD
eqn19 = -NForceC + NForceD + WCD == MassCD * accelerationS3;
eqn20 = cross(C-S3,-NForceC) + cross(D-S3,NForceD) == J_CD * alphaCD_vector;

% Equations for Link EF
eqn21 = -NForceE + NForceF + WEF == MassEF * accelerationS4;
eqn22 = cross(E-S4,-NForceE) + cross(F-S4,NForceF) == J_EF * alphaEF_vector;

% Equations for Link FG
eqn23 = -NForceF + NForceG + WFG + AppliedForce == MassFG * accelerationS5;
eqn24 = cross(F-S5,-NForceF) + cross(G-S5,NForceG) == J_FG * alphaFG_vector;

% Solving
eqnMatrixDynamics = [eqn15, eqn16, eqn17, eqn18, eqn19, eqn20, eqn21, eqn22, eqn23, eqn24];
NewtonSolution = solve(eqnMatrixDynamics, [NFAx NFAy NFBx NFBy NFCx NFCy NFDx NFDy NFEx NFEy NFFx NFFy NFGx NFGy NTin]);

% Extract forces from the Newton's second law solution
NForceA_x = double(NewtonSolution.NFAx);
NForceA_y = double(NewtonSolution.NFAy);
NForceB_x = double(NewtonSolution.NFBx);
NForceB_y = double(NewtonSolution.NFBy);
NForceC_x = double(NewtonSolution.NFCx);
NForceC_y = double(NewtonSolution.NFCy);
NForceD_x = double(NewtonSolution.NFDx);
NForceD_y = double(NewtonSolution.NFDy);
NForceE_x = double(NewtonSolution.NFEx);
NForceE_y = double(NewtonSolution.NFEy);
NForceF_x = double(NewtonSolution.NFFx);
NForceF_y = double(NewtonSolution.NFFy);
NForceG_x = double(NewtonSolution.NFGx);
NForceG_y = double(NewtonSolution.NFGy);
InputTorque_new = double(NewtonSolution.NTin);

disp(' ');
disp('Dynamic Joint Forces:');
disp(['Force A_x: ', num2str(NForceA_x)]);
disp(['Force A_y: ', num2str(NForceA_y)]);
disp(['Force B_x: ', num2str(NForceB_x)]);
disp(['Force B_y: ', num2str(NForceB_y)]);
disp(['Force C_x: ', num2str(NForceC_x)]);
disp(['Force C_y: ', num2str(NForceC_y)]);
disp(['Force D_x: ', num2str(NForceD_x)]);
disp(['Force D_y: ', num2str(NForceD_y)]);
disp(['Force E_x: ', num2str(NForceE_x)]);
disp(['Force E_y: ', num2str(NForceE_y)]);
disp(['Force F_x: ', num2str(NForceF_x)]);
disp(['Force F_y: ', num2str(NForceF_y)]);
disp(['Force G_x: ', num2str(NForceG_x)]);
